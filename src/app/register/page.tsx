    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-950 text-white font-sans">
      <div className="max-w-md w-full p-8 rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative text-center mb-10">
          <h1 className="text-4xl font-bold text-amber-500 mb-2 tracking-tighter uppercase">NUEVA CUENTA</h1>
          <p className="text-slate-400">Únete a la nueva era financiera</p>
        </div>

        <form onSubmit={handleRegister} className="space-y-5">
           <div>
            <label className="block text-sm font-medium text-slate-400 mb-2 tracking-widest uppercase text-[10px]">Nombre de Usuario</label>
            <input
              type="text"
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              className="w-full px-5 py-4 rounded-xl bg-slate-800/50 border border-slate-700 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all font-bold"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-black rounded-xl hover:from-amber-400 hover:to-amber-500 transform hover:scale-[1.02] transition-all disabled:opacity-50 shadow-xl shadow-amber-500/20"
          >
            {loading ? 'Creando Identidad...' : 'Empezar Inversión'}
          </button>
        </form>
